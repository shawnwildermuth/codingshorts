...
public class BookRepository(BookContext context) : IBookRepository
{
  ...
  public async Task<IEnumerable<BookEntryLookup>> GetLookupEntries()
  {
    var filter = "W";

    return await context.Database
      .SqlQuery<BookEntryLookup>($"""
        SELECT Id, FirstName, LastName, MiddleName, CompanyName
          FROM BookEntries
        --ORDER BY LastName, FirstName
        """)
      .Where(b => b.LastName.StartsWith(filter))
      .OrderBy(b => b.LastName)
      .ThenBy(b => b.FirstName)
      //.Where(b => b.UserName == Thread.CurrentPrincipal?.Identity?.Name)
      //.OrderBy(b => b.LastName)
