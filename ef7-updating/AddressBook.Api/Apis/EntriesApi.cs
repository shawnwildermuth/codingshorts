...
public class EntriesApi : IApi
{
  public void Register(IEndpointRouteBuilder builder)
  {
    ...
    grp.MapPost("fixemptygenders", FixGenders);

    grp.MapPut("{id:int}", PutEntry)
      .Validate<BookEntryModel>();

    grp.MapDelete("{id:int}", DeleteEntry);
    grp.MapDelete("deletewithoutaddresses", DeleteWithoutAddresses);
  }

  public async Task<IResult> DeleteWithoutAddresses(IBookRepository repository)
  {
    var result = await repository.DeleteEntriesWithoutAddresses();

    if (result) return Results.Ok(new { deleted = result });
    return Results.BadRequest();
  }

  public async Task<IResult> FixGenders(IBookRepository repository)
  {
    var result = await repository.FixEmptyGenders();
  ...
  {
    var result = await repository.DeleteEntry(id);
