...
public class BookRepository(BookContext context) : IBookRepository
{
  ...
  public async Task<bool> FixEmptyGenders()
  {
    return await context.BookEntries
      .Where(e => string.IsNullOrWhiteSpace(e.Gender))
      .ExecuteUpdateAsync(s => s.SetProperty(p => p.Gender, "Unknown")) > 0;
  }

  public async Task<bool> DeleteEntriesWithoutAddresses()
  {
    return await context.BookEntries
